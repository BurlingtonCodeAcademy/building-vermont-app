...
module.exports = {
  ...
  fetchAll: (params, next, { populate } = {}) => {
    ...
    const hook = strapi.hook[Page.orm];
    // Generate stages.
    const populateStage = hook.load().generateLookupStage(Page, { whitelistedPopulate: populate }); // Nested-Population
    const matchStage = hook.load().generateMatchStage(Page, filters); // Nested relation filter
    const aggregateStages = mergeStages(populateStage, matchStage);

    const result = Page.aggregate(aggregateStages)
      .skip(filters.start)
      .limit(filters.limit);

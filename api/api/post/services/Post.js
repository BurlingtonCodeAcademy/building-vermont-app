...
module.exports = {
  ...
  fetchAll: (params, next, { populate } = {}) => {
    ...
    // Generate stages.
    const populateStage = hook.load().generateLookupStage(Post, { whitelistedPopulate: populate }); // Nested-Population
    const matchStage = hook.load().generateMatchStage(Post, filters); // Nested relation filter
    const aggregateStages = mergeStages(populateStage, matchStage);

    const result = Post.aggregate(aggregateStages)
      .skip(filters.start)
      .limit(filters.limit);
